...
<style>
  ...
  /* Compact mode ------------------------------------------------------
     Section headings were consuming 53px each and the default 16px block gap
     added up fast. Tightening both is what makes a 3x3 card grid fit. */
  h4 {{ font-size: 15px !important; margin: 0 0 2px 0 !important;
        padding: 0 !important; line-height: 1.25 !important; }}
  h5, h6 {{ font-size: 12.5px !important; margin: 0 0 2px 0 !important;
            padding: 0 !important; line-height: 1.25 !important; }}
  [data-testid="stVerticalBlock"] {{ gap: 0.45rem; }}
  [data-testid="stHorizontalBlock"] {{ gap: 0.6rem; }}
  /* Zeroing element margins pulled the first row up under the tab bar and
     clipped it, so the panel supplies the clearance instead. */
  .stTabs [role="tabpanel"] {{ padding-top: 8px; }}

  /* Cards on the dense landing grid. */
  .cardhead {{
    font-size: 11px; font-weight: 600; letter-spacing: 0.04em;
    text-transform: uppercase; color: {p.MUTED};
    margin: 0 0 2px 2px; padding-top: 3px;
    line-height: 15px; min-height: 18px; overflow: visible;
  }}

...
def mini(fig, title, h=228, ylab=None, xlab=None, legend=False):
    """Shrink a figure to landing-card size.

    The title is drawn INSIDE the figure rather than as a separate st.markdown
    above it. Streamlit collapses the markdown element container to a few
    pixels here, so the chart that follows paints over the heading; putting the
    text in the figure sidesteps that entirely and costs less height."""
    fig.update_layout(
        height=h, margin=dict(l=4, r=6, t=30 if not legend else 44, b=4),
...
tabs = st.tabs(["Dashboard", "Overview", "Rate vs Volume", "Priority Schedule",
                "Burst Risk", "Loss Composition", "Plant Profile"])
(TAB_DASH, TAB_OVERVIEW, TAB_RATEVOL, TAB_SCHEDULE, TAB_BURST,
 TAB_COMPOSITION, TAB_PLANT) = tabs


# ==========================================================================
# TAB 0 — Dashboard (dense landing grid)
# ==========================================================================
with TAB_DASH:
    # Nine cards on one screen: every argument the dashboard makes, visible at
    # a glance. Each card is a stripped-down version of a chart that has a
    # full-size, fully-explained counterpart in the tab behind it.
    _n8 = min(8, n_plants)
    _rec_total = sel.physical_loss_m3.sum()

    r1 = st.columns(3)

    with r1[0]:
        d_ = sel.nlargest(_n8, "physical_loss_m3").sort_values("physical_loss_m3")
        f = go.Figure(go.Bar(
            x=d_.physical_loss_m3, y=d_.plant, orientation="h",
            marker=dict(color=d_.physical_loss_m3, colorscale=T.SEQ,
                        line=dict(color=T.SURFACE, width=1.5), showscale=False),
            text=[m3(v) for v in d_.physical_loss_m3], textposition="outside",
            textfont=dict(size=9, color=T.INK_2),
            customdata=d_.district,
            hovertemplate=("<b>%{y}</b> · %{customdata}<br>"
                           "%{x:,.0f} m³ recoverable<extra></extra>")))
        f.update_xaxes(range=[0, d_.physical_loss_m3.max() * 1.28])
        st.plotly_chart(mini(f, f"Recoverable water · top {_n8} plants"), width='stretch', config=PLOT_CFG, theme=None)

    with r1[1]:
        sv = sel.sort_values("physical_loss_m3", ascending=False).reset_index(drop=True)
        sv["cum"] = sv.physical_loss_m3.cumsum() / _rec_total * 100
        n10 = min(10, len(sv))
        ...
        f.add_vline(x=n10, line=dict(color=T.BASELINE, width=1, dash="dot"))
        f.add_annotation(x=n10, y=sv.cum.iloc[n10 - 1],
                         text=f"<b>{sv.cum.iloc[n10-1]:.0f}%</b> in {n10}",
                         showarrow=False, xshift=42, yshift=-12,
                         font=dict(size=10, color=T.INK))
        f.update_yaxes(range=[0, 104], ticksuffix="%")
        st.plotly_chart(mini(f, "Loss concentration"), width='stretch', config=PLOT_CFG, theme=None)

    with r1[2]:
        tr_, tv_ = sel.nsmallest(10, "rate_rank"), sel.nsmallest(10, "volume_rank")
        pl_ = sel.copy()
        pl_["grp"] = np.select(
        ...
        for g_, c_ in [("Neither", T.NEUTRAL), ("Top 10 volume", T.BLUE),
            ...
            f.add_trace(go.Scatter(
                ...
                marker=dict(size=np.sqrt(q.nrw_m3 / pl_.nrw_m3.max()) * 22 + 4,
        ...
        st.plotly_chart(mini(f, "Rate vs volume — the divergence", legend=True), width='stretch',
                        config=PLOT_CFG, theme=None)
    ...
    with r2[0]:
        ...
        f = go.Figure()
        f.add_trace(go.Bar(x=d_.physical_loss_m3, y=d_.plant, orientation="h",
                           name="Recoverable",
                           marker=dict(color=T.BLUE,
                                       line=dict(color=T.SURFACE, width=1.5)),
                           hovertemplate="<b>%{y}</b><br>%{x:,.0f} m³<extra></extra>"))
        f.add_trace(go.Bar(x=d_.commercial_loss_m3, y=d_.plant, orientation="h",
                           name="Not recoverable",
                           marker=dict(color=T.ORANGE,
                                       line=dict(color=T.SURFACE, width=1.5)),
                           hovertemplate="<b>%{y}</b><br>%{x:,.0f} m³<extra></extra>"))
        f.update_layout(barmode="stack")
        st.plotly_chart(mini(f, "Recoverable vs not recoverable", legend=True), width='stretch',
                        config=PLOT_CFG, theme=None)

    with r2[1]:
        d_ = (sel.groupby("district", as_index=False)
                 .agg(v=("physical_loss_m3", "sum"))
                 .sort_values("v"))
        f = go.Figure(go.Bar(
            ...
            marker=dict(color=d_.v, colorscale=T.SEQ,
                        line=dict(color=T.SURFACE, width=1.5), showscale=False),
            hovertemplate="<b>%{y}</b><br>%{x:,.0f} m³<extra></extra>"))
        st.plotly_chart(mini(f, "Recoverable water by district"), width='stretch', config=PLOT_CFG, theme=None)

    with r2[2]:
        mo = (msel.groupby("month", as_index=False)
                  .agg(n=("nrw_m3", "sum"), p=("production_m3", "sum"))
                  .assign(pct=lambda d: d.n / d.p * 100))
        names = ["Jan", "Feb", "Mar", "Apr", "May", "Jun",
                 "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
        f = go.Figure(go.Scatter(
            x=[names[i - 1] for i in mo.month], y=mo.pct, mode="lines+markers",
            line=dict(color=T.BLUE, width=2),
            marker=dict(size=5, color=T.BLUE),
            hovertemplate="<b>%{x}</b><br>%{y:.2f}%<extra></extra>"))
        span = mo.pct.max() - mo.pct.min()
        mid = mo.pct.mean()
        pad = max(span * 1.6, 1.5)
        f.update_yaxes(range=[mid - pad, mid + pad], ticksuffix="%")
        st.plotly_chart(mini(f, "Monthly loss rate"), width='stretch', config=PLOT_CFG, theme=None)
    ...
    with r3[0]:
        if HAS_BURST:
            ...
            if len(districts) < yearly.district.nunique():
                bp_ = bp_[bp_.district.isin(districts)]
            d_ = bp_.nsmallest(_n8, "risk_rank").sort_values("risk_pct")
            BC = {"Critical": T.CRITICAL, "High": T.SERIOUS,
                  "Moderate": T.WARNING, "Low": T.GOOD}
            f = go.Figure(go.Bar(
                x=d_.risk_pct, y=d_.plant, orientation="h",
                marker=dict(color=[BC.get(b, T.BLUE) for b in d_.risk_band],
                            line=dict(color=T.SURFACE, width=1.5)),
                text=[f"{v:.0f}%" for v in d_.risk_pct], textposition="outside",
                textfont=dict(size=9, color=T.INK_2),
                customdata=d_.risk_band,
                hovertemplate=("<b>%{y}</b><br>%{x:.0f}% · %{customdata}"
                               "<extra></extra>")))
            f.update_xaxes(range=[0, 118])
            st.plotly_chart(mini(f, f"Burst risk · {burst_metrics['horizon']}"), width='stretch', config=PLOT_CFG, theme=None)
        else:
            st.info("Run `python train_burst_model.py` to populate burst risk.")

    with r3[1]:
        if HAS_BURST:
            order = ["Low", "Moderate", "High", "Critical"]
            cnt = (burst_pred.risk_band.value_counts()
                   .reindex(order).fillna(0).astype(int))
            BC = {"Critical": T.CRITICAL, "High": T.SERIOUS,
                  "Moderate": T.WARNING, "Low": T.GOOD}
            f = go.Figure(go.Bar(
                x=order, y=cnt.values,
                marker=dict(color=[BC[b] for b in order],
                            line=dict(color=T.SURFACE, width=1.5)),
                text=cnt.values, textposition="outside",
                textfont=dict(size=10, color=T.INK_2),
                hovertemplate="<b>%{x}</b><br>%{y} plants<extra></extra>"))
            f.update_yaxes(range=[0, max(cnt.values) * 1.25])
            st.plotly_chart(mini(f, "Plants by risk band"), width='stretch', config=PLOT_CFG, theme=None)

    with r3[2]:
        ol = sel.sort_values("lips_rank")
        orr = sel.sort_values("rate_rank")
        x_ = np.arange(1, n_plants + 1)
        f = go.Figure()
        for nm, df_, c_ in [("LIPS / volume", ol, T.BLUE), ("Rate", orr, T.ORANGE)]:
            f.add_trace(go.Scatter(
                x=x_, y=df_.physical_loss_m3.cumsum() / _rec_total * 100,
                mode="lines", name=nm, line=dict(color=c_, width=2),
                hovertemplate=(f"<b>{nm}</b><br>first %{{x}} plants<br>"
                               "%{y:.0f}% of water<extra></extra>")))
        f.update_yaxes(range=[0, 104], ticksuffix="%")
        st.plotly_chart(mini(f, "Water reached, by queue order", legend=True), width='stretch',
                        config=PLOT_CFG, theme=None)
